      ...
      if (axios.isAxiosError(error) && error.response) {
        switch (error.response.status) {
          case 401:
            ...
            break;
          case 403:
            setEmsg('Please wait for your timeslot');
            break;
          case 500:
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <div className="flex-1 rounded-lg bg-gray-50 px-6 pb-4 pt-8">
        <h1 className={`${popins.className} mb-3 text-2xl`}>
          Please log in to continue.
        </h1>
        <div className="w-full">
          <div>
            <label
              className={`mb-3 mt-5 block text-xs font-medium text-gray-900 ${popins.className}`}
              htmlFor="username"
            >
              Username
            </label>
          ...
          <Button type="submit" className={`mt-4 flex items-center justify-center ${popins.className}`} >
            Login
